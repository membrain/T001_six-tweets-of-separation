...
If you are a MemBrain collaborator, clone the project with the private url:
	...
	prompt$ git clone git@github.com:membrain/T001_six-tweets-of-separation.git
...
The credentials to the Twitter API are stored in a YAML file located at ${RAILS-ROOT}/config/twitter.yml.
...
the credentials to use.  Or use any valid credentials you like.
...
branch, which makes integrations with the remote project less troublesome.  Use "git help" for 
more information on the branch and checkout commands.

Two bash scripts are included in the {RAILS-ROOT}/script/git folder of the project.  They are named
...
alias that runs hack, then rake (which runs all rails test defined to the project), then ship:
...
New Git users are encouraged to learn more about the rebase and merge commands to understand why this
